...
_unmap_reverse(keys_context_t *ctx, const char *key, const char *action)
{
    keys_reverse_binding_t *reverse
        = _find_reverse_binding(ctx->reverse_map, action);
...
_map_reverse(keys_context_t *ctx, const char *key, const char *action)
{
    ...
    reverse->keysyms
        = eina_list_append(reverse->keysyms, eina_stringshare_add(key));
}
...
{
    log(LIBKEYS_DEBUG_CONFIGS, "map key %s %s %s", context, key, action);
...
keys_load(keys_t *keys, const char *filename)
{
    ...
    if (!ini || !ini->data) {
        log(LIBKEYS_DEBUG_CONFIGS, "%s: can't load %s",keys->app_name,filename);
        if (ini)
            efreet_ini_free(ini);
        return NULL;
    }
...
keys_t *
keys_alloc(const char *app_name)
{
    eina_init();
...
const char *
keys_lookup(const keys_t *keys, const char *context, const char *key)
{
...
Eina_List *
keys_reverse_lookup(const keys_t *keys, const char *context, const char *action)
{
    keys_context_t *ctx = _find_context(keys, context);
    if (!ctx) {
        log(LIBKEYS_DEBUG_LOOKUP, "[%s] -> <no context>", context);
...
const char *
keys_lookup_by_event(const keys_t *keys, const char *context,
